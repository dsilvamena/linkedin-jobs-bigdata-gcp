...
Los diez scripts leen `linkedin_job_postings.csv`, `job_skills.csv` y
`job_summary.csv` desde `gs://utec-linkedin-jobs-2026/raw/`.
...
ejecuta Polars en un proceso Python, que lee los CSV directamente de GCS. Los
...
2. Instala `requirements.txt` en el master. En `w-0`, instala Polars y
   `google-auth` con el mismo Python que utiliza el proceso `dask worker`.
   ...
   ```bash
   python -m pip install 'polars>=1.44,<2' 'google-auth>=2,<3'
   ```
...
3. Inicia el scheduler en el master y el worker con el nombre `w-0` (si ya
   ...
   Hay más detalles del clúster en
   [`dask_proyecto/README.md`](../../dask_proyecto/README.md). En el master,
   ejecuta:
